...
## 1.20 development scope (experimental, not released)

The boundaries and controls below extend the 1.19.5 tables above with 1.20
Browser Platform scope. They have real execution and integration evidence for
their documented scope; no status is promoted by documentation alone. The
authoritative runtime-by-runtime status is `HK_CSP_SECURITY.md`.
...
| Boundary | Implemented control and residual risk |
|---|---|
| Edge deployment | Profiles apply bounded runtime limits. Native Linux ARM64 and macOS ARM64 CI cover the named runtime, security, memory, C++, and cpp-ml routes. No ARM64 release or unstable-network guarantee is claimed. |
...
| Threat | Required control | Current status |
|---|---|---|
| Eavesdropping | TLS 1.3 on public remote transport | experimental in Python/Node/Go/Rust; provider-dependent in C++ with OpenSSL (`HANDOFFKIT_WITH_TLS=ON`) |
| Replay/reordering | Peer/session nonce, sequence, timestamp | integrated; durable when configured in four secure runtimes |
| Security-profile downgrade | Exact selection plus authenticated transcript | integrated on Python/Node/Go/Rust TLS framed paths and C++ shared-vector verification; provider/session qualification remains experimental |
