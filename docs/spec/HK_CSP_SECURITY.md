...
| ARM64/edge profile | partially integrated | Shared `edge-small`, `edge-standard`, and `server` profiles apply real session/frame/retry limits in Python, JavaScript, Go, Rust, and the C++ local compute queue. The CI workflow contains a native Linux ARM64 job that exercises TLS, durable state, edge-small RSS, Rust, C++, cpp-ml CPU, and the Go gateway route. This branch has not yet produced a remote ARM64 result, so no ARM64 deployment or unstable-network qualification is claimed. |
