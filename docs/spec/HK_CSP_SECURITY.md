...
| ECDSA, ML-DSA, or SLH-DSA artifact signatures | unavailable | They are not allowlisted or advertised. Python/JavaScript/Rust surface `artifact_algorithm_unsupported` as a structured error before verification; Go/C++ already reject the same code. No sign/verify backend is present. |
