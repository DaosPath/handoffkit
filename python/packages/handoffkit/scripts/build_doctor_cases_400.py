"""Build a curated 400-case fixture from MedCaseReasoning.

The generated fixture is deterministic and uses the public test split only.
It is for educational benchmark evaluation, not medical advice.
"""
...
OUTPUT = Path("handoffkit/doctor_cases_400.py")
...
def build(limit: int = 400) -> list[dict[str, str]]:
    """Build deterministic case fixture rows."""
    ...
    with urllib.request.urlopen(request, timeout=120) as response:
        ...
        for index, row in enumerate(reader):
            ...
            selected.append(
                {
                    "case_id": f"medcase400-{len(selected):04d}",
...
def write_fixture(cases: list[dict[str, str]]) -> None:
    ...
    OUTPUT.write_text(
        "# ruff: noqa: E501\n"
        '"""Four hundred curated real open-access diagnostic cases.\n\n'
        ...
        '"""\n\n'
        "from __future__ import annotations\n\n"
        f"DOCTOR_CASES_400 = {json.dumps(cases, ensure_ascii=False, indent=2)}\n",
        encoding="utf-8",
    )
