...
- [`cpp/packages/handoffkit-ml/`](../handoffkit-ml/) — `handoffkit-ml` `0.6.0` (opt-in; **not** linked from core; direct TLS worker is provider-dependent)
- Product split: `handoffkit-cli train …` = jobs/distill; `handoffkit-ml sft|generate …` = native weights (CPU / CUDA / cuda-resident)
- **Status:** `cpp-ml` maintains an independent `0.6.0` line; its direct TLS worker and durable ledger route are experimental ([STATUS.md](../handoffkit-ml/STATUS.md)).
...
.\cpp\packages\handoffkit\build\example_team_handoff.exe
```
