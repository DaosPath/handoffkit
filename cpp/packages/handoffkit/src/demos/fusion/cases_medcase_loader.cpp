...
std::vector<BenchCase> load_or_builtin() {
    ...
    // New layout: python/packages/handoffkit/handoffkit/benchmarks/data/doctor_cases_30.json
    // Upward search (1.20 delta) adapted to the new layout.
    const auto fixture = std::filesystem::path("python") / "packages" / "handoffkit" /
