...
fn public_bind_rejected() {
    let error: BrowserCoreError = reject_public_bind(&serde_json::json!({}), "0.0.0.0").unwrap_err();
...
fn network_and_filesystem_policies() {
    assert_eq!(classify_network_target("http://127.0.0.1/")["kind"], "loopback");
