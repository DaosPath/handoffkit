...
def generate_fixtures(output: Path, repository: Path) -> None:
    generator = repository / "packages" / "contracts" / "test-fixtures" / "tls" / "generate.py"
...
def client_args(root: Path, runtime: str, port: int, tls_root: Path, worker: str, session: str) -> tuple[list[str], Path]:
    ...
    if runtime == "node":
        return [
            ...
            str(root / "packages" / "cpp-ml" / "tests" / "interop" / "node_tcp_client.mjs"),
            *common,
        ], root
    if runtime == "python":
        return [
            sys.executable,
            str(root / "packages" / "cpp-ml" / "tests" / "interop" / "python_tcp_client.py"),
            *common,
        ], root
    if runtime == "go":
        return [
    ...
    if runtime == "rust":
        return [
            ...
            "run", "--quiet", "--locked", "--manifest-path", str(root / "packages" / "rust" / "Cargo.toml"),
...
def server_args(root: Path, runtime: str, port: int, tls_root: Path) -> tuple[list[str], Path]:
    ...
    if runtime == "node":
        return [
            os.environ.get("HANDOFFKIT_NODE_BIN", "node"),
            str(root / "packages" / "cpp-ml" / "tests" / "interop" / "node_tcp_server.mjs"),
            *common,
        ], root
    if runtime == "python":
        return [
            sys.executable,
            str(root / "packages" / "cpp-ml" / "tests" / "interop" / "python_tcp_server.py"),
            *common,
        ], root
    if runtime == "go":
        return [
