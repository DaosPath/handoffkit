...
func TestRegistryAndSchedulerRecovery(t *testing.T) {
	...
	if !scheduler.Fail(first.AssignmentID, true) {
	...
	if count := scheduler.RecoverExpired(time.Now().Add(time.Hour)); count != 1 {
...
func TestSchedulerRetryNeverExceedsQueueCapacity(t *testing.T) {
	...
	if !scheduler.Fail(assignment.AssignmentID, true) {
