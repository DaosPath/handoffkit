...
func TestLengthDelimitedRealTCP(t *testing.T) {
	...
	go func() {
		...
		if acceptErr != nil { done <- acceptErr; return }
		server, wrapErr := NewLengthDelimited(connection, DefaultConfig())
		if wrapErr != nil { done <- wrapErr; return }
		envelope, receiveErr := server.Receive(context.Background())
		if receiveErr == nil { receiveErr = server.Send(context.Background(), envelope) }
		_ = server.Close()
		done <- receiveErr
	}()
	...
	if err := client.Send(context.Background(), envelope); err != nil { t.Fatal(err) }
	response, err := client.Receive(context.Background())
	if err != nil || response.MessageID != envelope.MessageID { t.Fatal("TCP roundtrip failed", err) }
	_ = client.Close()
...
func TestOversizedHeaderRejectedBeforePayload(t *testing.T) {
	...
	if _, err := client.Receive(context.Background()); err == nil { t.Fatal("oversized frame accepted") }
}
...
func TestTCPRetryConnectsWhenServerAppears(t *testing.T) {
	...
	go func() {
		...
		if listenErr != nil { ready <- nil; return }
		ready <- listener
	...
	_ = client.Close()
	listener := <-ready
	if listener == nil { t.Fatal("server failed to start") }
	listener.Close()
}
