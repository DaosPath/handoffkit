...
func TestChannelFIFOBackpressureAndClose(t *testing.T) {
	...
	if err := channel.Send(context.Background(), envelope("session", 1)); err != nil { t.Fatal(err) }
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Millisecond)
	defer cancel()
	if err := channel.Send(ctx, envelope("session", 2)); err == nil { t.Fatal("blocked send did not honor cancellation") }
	first, err := channel.Receive(context.Background())
	if err != nil || first.Sequence != 1 { t.Fatalf("FIFO failed: %#v %v", first, err) }
	channel.Close()
	channel.Close()
	if err := channel.Send(context.Background(), envelope("session", 3)); err == nil { t.Fatal("send after close succeeded") }
}
...
func TestSessionDedupACKAndRetryRelease(t *testing.T) {
	...
	if err := session.Send(context.Background(), "tasks", message); err != nil { t.Fatal(err) }
	received, err := session.Receive(context.Background(), "tasks")
	if err != nil { t.Fatal(err) }
	ack := session.Ack(received, map[string]any{})
	if ack.MessageID != message.MessageID { t.Fatal("ACK confirmed another message") }
	if err := session.Send(context.Background(), "tasks", message); err != nil { t.Fatal(err) }
	unique := envelope("session", 2)
	if err := session.Send(context.Background(), "tasks", unique); err != nil { t.Fatal(err) }
	received, err = session.Receive(context.Background(), "tasks")
	if err != nil || received.MessageID != unique.MessageID { t.Fatal("duplicate was executed") }
	if _, err := session.Nack(received, "retry", "retry safely", true, map[string]any{}); err != nil { t.Fatal(err) }
	if err := session.Send(context.Background(), "tasks", unique); err != nil { t.Fatal(err) }
	if _, err := session.Receive(context.Background(), "tasks"); err != nil { t.Fatal(err) }
}
...
func TestFileDedupPersists(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if fresh, err := first.Claim("key"); err != nil || !fresh { t.Fatal("first claim failed", err) }
	second, err := NewFileDedupStore(path, 16, 1024*1024)
	if err != nil || !second.Contains("key") { t.Fatal("claim did not persist", err) }
	if released, err := second.Release("key"); err != nil || !released { t.Fatal("release failed", err) }
}
...
func TestConcurrentSmoke(t *testing.T) {
	...
	for sequence := 0; sequence < messages; sequence++ {
		...
		go func(sequence int) {
			...
			if err := session.Send(context.Background(), "tasks", envelope("stress", uint64(sequence+1))); err != nil { t.Error(err) }
		}(sequence)
	}
	...
	producers.Wait()
	if len(seen) != messages { t.Fatalf("lost messages: %d", len(seen)) }
}
...
func TestConcurrentDuplicateClaimExecutesOnce(t *testing.T) {
	...
	for index := 0; index < duplicates; index++ {
		...
		if err := session.Send(context.Background(), "tasks", copy); err != nil { t.Fatal(err) }
	}
...
func TestConcurrentEnvelopeSequencesAreUnique(t *testing.T) {
	...
	for value := range values { seen[value] = struct{}{} }
	if len(seen) != count { t.Fatalf("sequence collision: got %d unique values", len(seen)) }
}
